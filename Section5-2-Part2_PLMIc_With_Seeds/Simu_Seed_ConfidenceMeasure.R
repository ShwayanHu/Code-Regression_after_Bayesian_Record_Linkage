...
# Loops -------------------------------------------------------------------
# looping variables
looping_grid <- expand.grid(0:5, c(1), c(0.5), c(0.3), c(0.01))
...
mclapply(
  ...
  function(loop_var) {
    ...
    for (dataset_var in 1:5) {
      ...
      Z_nonlink <- preprocessed_data[[dataset_var]]$Z_nonlink
      ...
      EM_and_TSols_history <- lapply(
        101:120, # first 100 iters are burn-in
        function(s){
          print(s)
          ...
          m_tmp <- m[[s]]
          u_tmp <- u[[s]]
          linked_data_tmp <- z2regdata_with_goodness_and_seed(Z_tmp, file_1, file_2, m_tmp, u_tmp, gamma, Z_known, Z_nonlink)
          X_tmp <- linked_data_tmp$X
          Y_tmp <- linked_data_tmp$Y
          G_tmp <- linked_data_tmp$G
          Seed.vec_tmp <- linked_data_tmp$Seed.vec
          EM_result <- EM_estimation_Seed_ConfidenceMeasure(
      ...
      save(
        ...
        file=paste(
          PROJECT_PATH,
          "Section5-2-Part2_PLMIc_With_Seeds/Simu_Result/",
          "RegResult",
          "_dataset",
          dataset,
          "(",
