...
EM_estimation_Seed_ConfidenceMeasure_3covariates <- function(X1, X2, X3, Y, G, p.y, Seed.vec) {
  # Helper functions
  ...
  phi <- function(y, x1, x2, x3, beta0, beta1, beta2, beta3, sigma) {
  ...
  log_likelihood.1 <- function(param, p.y) {
    ...
    term.1 <- (2*pi*sigma^2)^(-1/2) * exp(-1/(2*sigma^2) * (y-beta_0-beta_1*x1-beta_2*x2-beta_3*x3)^2) * h(g, eta_0, eta_1)
    term.2 <- p.y(y) * (1 - h(g, eta_0, eta_1))
    return(log(term.1 + term.2))
  }
  
  # E-step function
  prob_t_is_1 <- function(x1, x2, x3, y, g, par, p.y) {
    beta_0 <- par[1]
    beta_1 <- par[2]
    beta_2 <- par[3]
    beta_3 <- par[4]
    sigma <- max(par[5], 1e-4)
    eta_0 <- par[6]
    eta_1 <- par[7]
    
    h_val <- h(g, eta_0, eta_1)
    phi_val <- pmax(phi(y, x1, x2, x3, beta_0, beta_1, beta_2, beta_3, sigma), 1e-8)
  ...
  Q <- function(X1, X2, Y, G, T_Estep, par) {
    ...
    theta_2 <- par[3]
    theta_3 <- par[4]
    sigma <- max(par[5], 1e-4)  # enforce positivity
    eta_0 <- par[6]
    eta_1 <- par[7]
    
    h_val <- h(G, eta_0, eta_1)
    phi_val <- pmax(phi(Y, X1, X2, X3, theta_0, theta_1, theta_2, theta_3, sigma), 1e-8)  # avoid log(0)
  ...
  # Initialization
  # browser()
  fit_lm <- lm(Y[Seed.vec==1] ~ X1[Seed.vec==1] + X2[Seed.vec==1] + X3[Seed.vec==1])
  # fit_lm <- lm(Y ~ X1 + X2 + X3)
  par_iter <- as.numeric(
    c(fit_lm$coefficients, sqrt(sum(fit_lm$residuals^2)/fit_lm$df.residual), 0, 0)
  )
  par_iter[3] <- -0.5; par_iter[1] <- 0.7
  par_prev <- rep(0, 7)
  iter <- 1
  
  # EM iterations
  while (sqrt(sum((par_iter - par_prev)^2)) > 1e-3 & iter <= 2000) {
    # E-step
    t <- prob_t_is_1(X1, X2, X3, Y, G, par_iter, p.y); t[Seed.vec==1] <- 1 # Force the latent class probability of seed to be 1
    ...
    # M-step
    fit1 <- optim(par = par_iter[1:5], fn = Q_regression, method = "BFGS", X1 = X1, X2 = X2, X3 = X3, Y=Y, T_Estep=t)  # M1-step
    fit2 <- optim(par = par_iter[6:7], fn = Q_logistic, method = "BFGS", G=G, T_Estep=t)  # M2-step
  ...
  complete_score <- function(par, X1, X2, X3, Y, G) {
    beta0 <- par[1]
    beta1 <- par[2]
    beta2 <- par[3]
    beta3 <- par[4]
    ...
    cbind(d_beta0, d_beta1, d_beta2, d_beta3, d_sigma, d_eta0, d_eta1)
  }
  
  # Complete data information
  complete_info <- function(par, X1, X2, X3, Y, G) {
    beta0 <- par[1]
    beta1 <- par[2]
    beta2 <- par[3]
    beta3 <- par[4]
    ...
    mu <- beta0 + beta1 * X1 + beta2 * X2 + beta3 * X3
    h_val <- h(G, eta0, eta1)
    t <- prob_t_is_1(X1, X2, X3, Y, G, par, p.y)
    
    ## --- Regression block (β0, β1, β2) ---
    Xmat <- cbind(1, X1, X2, X3)   # design matrix
    I_beta <- matrix(0, 4, 4)
    for (i in 1:length(Y)) {
  ...
  scores <- complete_score(par_iter, X1, X2, X3, Y, G)
  outer_prod <- apply(scores, 1, function(x) x %*% t(x))
  B <- matrix(rowSums(outer_prod), 7, 7)
  C <- complete_info(par_iter, X1, X2, X3, Y, G)
