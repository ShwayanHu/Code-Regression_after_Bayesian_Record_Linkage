...
// [[Rcpp::export]]
List update_m_cpp(List m, List Z, DataFrame gamma, IntegerVector L, List alpha) {
  ...
  for (int f = 1; f <= 4; ++f) {
    ...
    for (int l = 0; l <= max_level; ++l) {
      for (int i = 0; i < n_gamma; ++i) {
        if (gamma_f[i] == l) {
          ...
          IntegerVector Z_j = Z[idx_j - 1];  // adjust for 0-based index
    ...
    NumericVector alpha_f = alpha[field];
    NumericVector dirichlet_param = count + alpha_f;
    
    m[field] = rdirichlet_cpp(dirichlet_param);
  }
