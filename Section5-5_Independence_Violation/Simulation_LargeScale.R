...
# Loops -------------------------------------------------------------------
# looping variables
looping_grid <- expand.grid(0:99, c(1), c(0.5), c(0.3), c(0.05), c(1, 3, 5))
...
lapply(
  ...
  function(loop_var) {
    ...
    mean_x_nonlink <- looping_grid[loop_var, 6]
    ...
    preprocessed_data <- PreProcess_SimulationData_withSeed_IndepViolation(
      records = records,
      num_simu = 5,
      n1 = 500,
      n2 = 500,
    ...
    for (dataset_var in 1:5) {
      ...
      if (
        result_file_name %in%
          list.files(paste0(PROJECT_PATH, "Section5-5_Independence_Violation/Simu_Result"))
      ) {
        next
      }
      ...
      EM_and_TSols_history <- lapply(
        101:110, # first 100 iters are burn-in
        function(s) {
          print(s)
          ...
          m_tmp <- m[[s]]
          u_tmp <- u[[s]]
          linked_data_tmp <- z2regdata_with_goodness_and_seed(Z_tmp, file_1, file_2, m_tmp, u_tmp, gamma, Z_known, Z_nonlink=NULL)
          X_tmp <- linked_data_tmp$X
          Y_tmp <- linked_data_tmp$Y
          G_tmp <- linked_data_tmp$G
          Seed.vec_tmp <- linked_data_tmp$Seed.vec
          # PLMIc
          PLMIc_result <- EM_estimation_Seed_ConfidenceMeasure(
          ...
          # PLMI
          PLMI_result <- EM_estimation_withSeed(
            X = as.numeric(X_tmp[,2]),
            Y = as.numeric(Y_tmp),
            Seed.vec = as.numeric(Seed.vec_tmp),
            p.y = function(y){dnorm(y, mean = mu.y, sd = sigma.y)}
          ...
          TS.OLS_coefficients <- solve(t(X_tmp) %*% X_tmp) %*%
            t(X_tmp) %*%
            Y_tmp
          TS.OLS_cov <- sum((Y_tmp - X_tmp %*% TS.OLS_coefficients)^2) /
            (nrow(X_tmp) - ncol(X_tmp)) *
            solve(t(X_tmp) %*% X_tmp)
          res <- list(
