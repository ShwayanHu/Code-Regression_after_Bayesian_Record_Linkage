...
# looping variable --------------------------------------------------------
looping_grid <- expand.grid(0:99, c(1, 3), c(0.5, 1), c(0.3, 0.6, 0.9), 1:5)
...
inference_df <- map_dfr(
  ...
  function(loop_var) {
    ...
    load(
      paste0(
        "~/BayesianRecordLinkage/Section5-2-Part1_PLMIc_Without_Seeds/Simu_Result/",
        result_file_name
      ),
      temp_env <- new.env()
    )
    ...
    # TS.OLS regression
    beta_0.TS.OLS.estimates <- sapply(1:9e2, function(i) {
      result_data[["res"]][["EM_and_TSols_result"]][[i]][[
        "TS.OLS_coefficients"
      ]][1]
    })
    beta_0.TS.OLS.var <- sapply(1:9e2, function(i) {
    ...
    beta_1.TS.OLS.estimates <- sapply(1:9e2, function(i) {
      result_data[["res"]][["EM_and_TSols_result"]][[i]][[
        "TS.OLS_coefficients"
      ]][2]
    })
    beta_1.TS.OLS.var <- sapply(1:9e2, function(i) {
    ...
    # EM results
    beta_0.EM.estimates <- sapply(1:900, function(i) {
      result_data[["res"]][["EM_and_TSols_result"]][[i]][["EM_result"]][[
        "par"
      ]][1]
    })
    beta_0.EM.var <- sapply(1:900, function(i) {
      result_data[["res"]][["EM_and_TSols_result"]][[i]][["EM_result"]][[
        "laplace_variance"
      ]][1, 1]
    })
    ...
    beta_1.EM.estimates <- sapply(1:900, function(i) {
      result_data[["res"]][["EM_and_TSols_result"]][[i]][["EM_result"]][[
        "par"
      ]][2]
    })
    beta_1.EM.var <- sapply(1:900, function(i) {
      result_data[["res"]][["EM_and_TSols_result"]][[i]][["EM_result"]][[
        "laplace_variance"
      ]][2, 2]
    })
...
result.beta1 <- inference_df %>%
  group_by(n_error, overlap_proportion, R_sq) %>%
  summarise(
